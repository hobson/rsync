...
# Python counterpart of rsync written by Vivian De Smedt
...
def visit(cookie, dirname, names):
    ...
    Example edge cases for testing on a Windows machine:
    (C:\Cvs -> C:\)!
    (C:\Archives\MyDatas\UltraEdit -> C:\Archives\MyDatas)
    (Cvs -> "")!
    (Archives\MyDatas\UltraEdit -> Archives\MyDatas)
    (\Cvs -> \)! (\Archives\MyDatas\UltraEdit -> Archives\MyDatas)
    """
    if os.path.split(cookie.sink_root)[1]:
    ...
    # TODO: `return filter_filenames(cookie, names, ...)`
    names_excluded = []
    if filters:
    ...
    # TODO: `return visit_filenames(cookie, names, ...)`
    for name in names:
...
#######################################################################
# TODO: Break up overly-complex `visit()` function above


def filter_filenames(cookie, dirname, names, filters, sink_dir, target_dir, names_excluded):
    # filter sink files (names):
    name_index = 0
    while name_index < len(names):
        name = names[name_index]
...
def visit_filenames(names, dirname, sink_dir, cookie, target_dir):
    for name in names:
        # Copy files and folder from sink to target.
        sink = os.path.join(sink_dir, name)
        # print sink
...
# END WIP: modularization of complex `visit` function above
###############################################################################
...
def main(args):
    ...
    opts, args = getopt.getopt(args, "qrRntuCIh",
        ["quiet", "recursive", "relative", "dry-run", "time", "update", "cvs-ignore", "ignore-times",
         "help", "delete", "delete-excluded", "delete-from-source", "existing", "size-only",
         "modify-window=", "exclude=", "exclude-from=", "include=", "include-from=", "version"])
    for o, v in opts:
        if o in ["-q", "--quiet"]:
            cookie.quiet = 1
        if o in ["-r", "--recursive"]:
